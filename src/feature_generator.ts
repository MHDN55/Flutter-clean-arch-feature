...
// Dart code templates for generated files:
...
@LazySingleton(as: ${pascalFeature}Repo)
...
function getRemoteDataSourceImplFileContent(pascalFeature: string, snakeFeature: string) {
    return `import 'package:injectable/injectable.dart';
...
// Main command function: create folders + files with content
export async function createFeatureFolders() {
    ...
    const featureName = await vscode.window.showInputBox({ prompt: "Enter the feature name (e.g., login, auth):" });
    if (!featureName) {
    ...
    // Files and their folder mapping
    const fileMap: { [fileName: string]: string } = {
        [`${snakeFeature}_repo.dart`]: path.join(baseFeaturePath, 'domain', 'repo'),
        [`${snakeFeature}_repo_impl.dart`]: path.join(baseFeaturePath, 'data', 'repo'),
        [`${snakeFeature}_remote_data_source.dart`]: path.join(baseFeaturePath, 'data', 'datasources', 'remote'),
        [`${snakeFeature}_remote_data_source_impl.dart`]: path.join(baseFeaturePath, 'data', 'datasources', 'remote'),
    };
    ...
    for (const [fileName, dirPath] of Object.entries(fileMap)) {
        ...
        if (!fs.existsSync(filePath)) {
            ...
            if (fileName.endsWith('_repo.dart')) {
                ...
                content = getRemoteDataSourceImplFileContent(pascalFeature, snakeFeature);
            }
